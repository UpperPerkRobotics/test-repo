...
task shooter_power_control()
{
	...
	while(true)
	{
		...
		if(shooter_target_speed == 0){
			...
			right_power = shooter_target_power;
		}
		...
		if(shooter_target_speed == 0){
			...
			left_power = shooter_target_power;
		}
		...
		// Save values for next reading time.
			...
			if(print_value == 1){
				...
				writeDebugStreamLine("Current Right Shooter Speed: %d", current_right_clicks);
				writeDebugStreamLine("Current Left Shooter Speed: %d", current_left_clicks);
...
void set_shooter_targets(int speed, int power){
...
void adjust_shooter_targets(int adjust_speed, int adjust_power){
